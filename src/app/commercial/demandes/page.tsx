...
export default function CommercialDemandesPage() {
  ...
  const reload = useCallback(async () => {
    ...
    setRows(all.filter((b) => PENDING_STATUSES.includes(b.status)));
  }, []);
  ...
  return (
    <>
      <SjSectionHead eyebrow="Commercial" title="Demandes clients" />
          ...
          <SjCard key={b.id}>
            <div className="sj-between">
              <div>
                ...
                <div className="sj-muted">{b.client?.full_name || b.client?.company_name || "Client"} · {b.pickup} → {b.dropoff}</div>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div className="sj-gold" style={{ marginTop: 8 }}>{b.estimated_price != null ? formatFcfa(b.estimated_price) : "Sur devis"}</div>
        ...
        <SjCard style={{ marginTop: 16 }}>
          ...
          <div className="sj-form-grid" style={{ marginTop: 12 }}>
            <div className="sj-field">
              ...
              <input type="number" value={quoteAmount} onChange={(e) => setQuoteAmount(e.target.value)} placeholder={selected.estimated_price?.toString() || "Ex. 30000"} />
            </div>
            <div className="sj-field">
              <label>Note interne</label>
              <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Commentaire commercial…" />
            </div>
          </div>
